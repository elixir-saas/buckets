defmodule Buckets.Router do
  ...
  - `GET /__buckets__/<encoded_bucket>/*path` - Download files
  - `PUT /__buckets__/<encoded_bucket>/*path` - Upload files

  The bucket name is base64-encoded and embedded in the route at compile time.

  ## Security

  These routes validate that upload requests have valid signatures (generated by
  the Volume adapter).
  ...
  The following routes are created (bucket is base64-encoded at compile time):
  - `GET /__buckets__/<encoded_bucket>/*path` - Downloads a file from the local filesystem
  - `PUT /__buckets__/<encoded_bucket>/*path` - Uploads a file to the local filesystem
  ...
  to ensure uploads include valid signatures.
  """
  defmacro buckets_volume(cloud_module, opts \\ []) do
    scope = "/" <> @scope

    quote bind_quoted: [scope: scope, cloud_module: cloud_module, opts: opts] do
      bucket = cloud_module.config()[:bucket] |> Base.url_encode64(padding: false)

      scope scope do
        private = %{cloud_module: cloud_module, opts: opts}
